<?php
/*
Plugin Name: Gravity Forms TransactionExpress Add-On
Plugin URI: http://www.gravityforms.com
Description: Based on Gravity Forms Authorize.Net Add-On. Cannot live side by side with Gravity Forms Authorize.Net Addon. Integrates Gravity Forms with TransAction Express by TransFirst LLC, enabling end users to purchase goods and services through Gravity Forms.
Version: 1.5
Author: rocketgenius, modified by Essential IT
...
class GFAuthorizeNet {
    ...
    private static function capture_product_payment($config, $entry, $form){
        ...
        if(self::$transaction_response["auth_transaction_id"]) // ($response->approved)
        {
    ...
    private static function get_initial_transaction($form_data, $config, $invoice_number=""){
        ...
        $transaction->invoice_num = empty($form_data['invoice_num']) ? uniqid() : $form_data['invoice_num'];
